...
class Products extends Component {
  ...
  callbackFunction(id, checked) {
    if (checked == true) this.setState({ id: [...this.state.id, id] });
    else {
      const filteredArray = this.state.id.filter((e) => e !== id);
  ...
  handleChangeSwitcher(event) {
    if (event.target.value == 'All products' || event.target.value == 'Все продукты' || event.target.value == 'Visi produkti') {
      ...
      this.state.category_ids = [];
      this.state.userGroupCategories.map((userGroupCategory) => this.state.userGroups.map((userGroup) => {
        if (event.target.value == userGroup.name_en || event.target.value == userGroup.name_ru || event.target.value == userGroup.name_lv) if (userGroup.id == userGroupCategory.user_group_id) this.state.category_ids.push(userGroupCategory.category_id);
      }), this.reload());
    }
        ...
        <div className="col-md-3 ml-2">
          ...
          <select className="browser-default custom-select" id="switcher" name="userGroupName" onChange={this.handleChangeSwitcher}>
            ...
            {this.state.userGroups.map((userGroup) => {
              const userGroupName = this.renderSwitch(i18n.language, userGroup);
